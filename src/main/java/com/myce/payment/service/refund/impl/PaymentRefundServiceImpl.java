...
/*
  - PortOne API / PaymentRepository / RefundRepository 삭제
  - RefundInternalService로 환불 처리 위임
  - 응답의 targetType/targetId/isPartial로 PaymentInfo 상태 업데이트
    기존:
    core → PortOne 환불 호출 → Refund 저장 → PaymentInfo 상태 변경
    변경 후:
    core → payment internal에 환불 요청 → 응답 받음 → PaymentInfo 상태만 변경
*/
@Slf4j
@Service
...
public class PaymentRefundServiceImpl implements PaymentRefundService {
    ...
    // core에서 유지해야 하는 PaymentInfo 상태 갱신용
    ...
    public RefundInternalResponse processAdRefund(AdRefundRequest request) {
        ...
        try {
            notificationService.sendAdvertisementStatusChangeNotification(
                    request.getAdId(), advertisement.getTitle(), oldStatus, advertisement.getStatus().name());
        } catch (Exception e) {
            log.warn("광고 환불 승인 알림 전송 실패 - adId: {}, 오류: {}", request.getAdId(), e.getMessage());
        }

        // 6) PaymentInfo 상태만 업데이트 (부분/전체 여부는 internal 응답 기준)
        updatePaymentInfoStatus(response);

        return response;
    }

    // RefundInternalResponse의 targetType/targetId/isPartial로 PaymentInfo 상태 업데이트
        ...
        switch (response.getTargetType()) {
            case RESERVATION:
                ReservationPaymentInfo rpi = reservationPaymentInfoRepository
                        .findByReservationId(response.getTargetId())
            ...
            case AD:
                AdPaymentInfo api = adPaymentInfoRepository
                        .findByAdvertisementId(response.getTargetId())
                        .orElseThrow(() -> new CustomException(CustomErrorCode.PAYMENT_INFO_NOT_FOUND));
            ...
            case EXPO:
                ExpoPaymentInfo epi = expoPaymentInfoRepository
                        .findByExpoId(response.getTargetId())
                        .orElseThrow(() -> new CustomException(CustomErrorCode.PAYMENT_INFO_NOT_FOUND));
